...
driver.get('https://search.naver.com/search.naver?where=image&sm=tab_jum&query=')#구글 이미지 주소 입력
element=driver.find_element_by_name("query")#네임 클래스 id 등으로 검색 가능. q를 입력하는 이유는 html상 name이 q로 지정되어있다.
# 사이트를 오픈했고 검색창이 뭔지 찾아냈다.


#html 설명
element.send_keys("빈센조") #키보드 입력값
...
images=driver.find_elements_by_css_selector("._image")#.rg_i.q4luwd 이미지 클래스명
count=1
